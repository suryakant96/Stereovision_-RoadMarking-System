function [Roadwidth,position,everypointdist] = Roadwidthvideo(LeftBorderPoints,RightBorderPoints,disparityMap,points3D)
...
dist=dist-0.009; %it is type of measuring instrument calibration
everypointdist=dist;
distance=(sum(dist)/length(dist))*100;
Roadwidth= distance/2; % this is distance in meeters our scale(2cm=1m)
position=pos;
end
