function  [img,leftx,rightx,yaxis]=Drawlanevid(LeftBorderPoints,RightBorderPoints,frameLeftRect,position,everypointdist,distance,disparityMap,points3D)
...
LaneX=lane;
leftx=LeftBorderPoints(position(end/2:end));
yaxis=position(end/2:end);
dis=100*everypointdist(yaxis(1));
rightx=RightBorderPoints(position(end/2:end));
